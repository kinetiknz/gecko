...
public:
  ...
  virtual nsresult DecodeHeader(const unsigned char* aData, size_t aLength);
  virtual nsresult FinishInit(AudioInfo& aInfo);
  virtual bool Decode(const unsigned char* aData, size_t aLength,
                      int64_t aOffset, uint64_t aTstampUsecs,
                      int32_t* aTotalFrames, nestegg_packet* aPacket);
  VorbisDecoder(WebMReader* aReader);
                      ...
                      int32_t* aTotalFrames, nestegg_packet* aPacket)
{
  ...
  const MediaInfo& info = mReader->GetMediaInfo();
  ...
  if (frames == 0 && first_packet) {
    mReader->AudioQueue().Push(new AudioData(aOffset, aTstampUsecs, 0, 0, nullptr,
                                             info.mAudio.mChannels,
                                             info.mAudio.mRate));
  }
  while (frames > 0) {
    uint32_t channels = info.mAudio.mChannels;
...
public:
  ...
  virtual bool Decode(const unsigned char* aData, size_t aLength,
                      int64_t aOffset, uint64_t aTstampUsecs,
                      int32_t* aTotalFrames, nestegg_packet* aPacket);
  OpusDecoder(WebMReader* aReader);
  virtual ~OpusDecoder();
private:
  nsRefPtr<WebMReader> mReader;
                    ...
                    int32_t* aTotalFrames, nestegg_packet* aPacket)
{
  ...
  int32_t samples =
    opus_packet_get_samples_per_frame(aData, opus_int32(info.mAudio.mRate));
  ...
  if (mSkip > 0) {
    ...
    PodMove(buffer.get(),
            ...
            keepFrames * channels);
    startTime = startTime + FramesToUsecs(skipFrames, info.mAudio.mRate);
    frames = keepFrames;
    mSkip -= skipFrames;
  }
