...
struct auto_com {
  ...
  ~auto_com() {
    if (result == RPC_E_CHANGED_MODE) {
      ...
      LOG(nullptr, "COM was already initialized in STA.\n");
      ...
      LOG(nullptr, "COM was already initialized in MTA\n");
    }
    if (SUCCEEDED(result)) {
      CoUninitialize();
    }
...
public:
  ...
  {
    LOG(this, "Audio device default changed.\n");
    ...
    if (!ok) {
      LOG(this, "SetEvent on reconfigure_event failed: %x\n", GetLastError());
    }
  ...
  {
    LOG(this, "Audio device state changed.\n");
    return S_OK;
  }
  ...
  {
    LOG(this, "Audio device property value changed.\n");
    return S_OK;
  }
private:
...
bool get_input_buffer(cubeb_stream * stm)
{
  ...
  while (offset != total_available_input) {
    ...
    if (FAILED(hr)) {
      LOG(stm, "cannot get next packet size: %x\n", hr);
      return false;
    }
    ...
    hr = stm->capture_client->GetBuffer(&input_packet,
                                        ...
                                        NULL);
    if (FAILED(hr)) {
      LOG(stm, "GetBuffer failed for capture: %x\n", hr);
      return false;
    }
    XASSERT(packet_size == next);
    if (flags & AUDCLNT_BUFFERFLAGS_SILENT) {
      LOG(stm, "insert silence: ps=%u\n", packet_size);
    ...
    if (FAILED(hr)) {
      LOG(stm, "FAILED to release intput buffer");
      return false;
    }
    offset += packet_size;
  }
...
bool get_output_buffer(cubeb_stream * stm, size_t max_frames, float *& buffer, size_t & frame_count)
{
  ...
  if (FAILED(hr)) {
    LOG(stm, "Failed to get padding: %x\n", hr);
    return false;
  }
  ...
  if (stm->draining) {
    ...
    if (FAILED(hr)) {
      LOG(stm, "GetFrequency failed: %x\n", hr);
    }

    UINT64 pos;
    hr = stm->audio_clock->GetPosition(&pos, NULL);
    if (FAILED(hr)) {
      LOG(stm, "GetPosition failed: %x\n", hr);
    }

    UINT64 fpos = pos * stm->output_mix_params.rate / freq;

    LOG(stm, "draining: padding_out=%u fpos=%llu maxpos=%llu\n", padding_out, fpos, stm->frames_written);
    if (fpos >= stm->frames_written) {
      stm->state_callback(stm, stm->user_ptr, CUBEB_STATE_DRAINED);
      return false;
    }
#else
    LOG(stm, "draining: padding_out=%u\n", padding_out);
    if (padding_out == 0) {
      stm->state_callback(stm, stm->user_ptr, CUBEB_STATE_DRAINED);
      return false;
    }
  ...
  if (FAILED(hr)) {
    LOG(stm, "cannot get render buffer\n");
    return false;
  }
...
refill_callback_duplex(cubeb_stream * stm)
{
  ...
  if (input_duration < output_duration) {
    ...
    LOG(stm, "padding silence: out=%f in=%f pad=%u\n", output_duration, input_duration, padding);
  ...
  hr = stm->render_client->ReleaseBuffer(output_frames, 0);
  if (FAILED(hr)) {
    LOG(stm, "failed to release buffer: %x\n", hr);
    return false;
  }
  return true;
}
...
wasapi_stream_render_loop(LPVOID stream)
{
  ...
  auto_com com;
  if (!com.ok()) {
    LOG(stm, "COM initialization failed on render_loop thread.\n");
  ...
  if (!mmcss_handle) {
    ...
    LOG(stm, "Unable to use mmcss to bump the render thread priority: %x\n", GetLastError());
  }
  ...
  while (is_playing) {
    ...
    case WAIT_TIMEOUT:
      ...
      break;
    default:
      LOG(stm, "case %d not handled in render loop.", waitResult);
      abort();
    }
  }
...
HRESULT register_notification_client(cubeb_stream * stm)
{
  ...
  if (FAILED(hr)) {
    LOG(stm, "Could not get device enumerator: %x\n", hr);
    return hr;
  }
  ...
  if (FAILED(hr)) {
    LOG(stm, "Could not register endpoint notification callback: %x\n", hr);
...
HRESULT get_endpoint(IMMDevice ** device, LPCWSTR devid)
{
  ...
  if (FAILED(hr)) {
    LOG(nullptr, "Could not get device enumerator: %x\n", hr);
    return hr;
  }

  hr = enumerator->GetDevice(devid, device);
  if (FAILED(hr)) {
    LOG(nullptr, "Could not get device: %x\n", hr);
...
HRESULT get_default_endpoint(IMMDevice ** device, EDataFlow direction)
{
  ...
  if (FAILED(hr)) {
    LOG(nullptr, "Could not get default audio endpoint: %x\n", hr);
    SafeRelease(enumerator);
    return hr;
  }
...
stream_set_volume(cubeb_stream * stm, float volume)
{
  ...
  if (hr != S_OK) {
    LOG(stm, "could not get the channel count: %x\n", hr);
    return CUBEB_ERROR;
  }
  ...
  if (hr != S_OK) {
    LOG(stm, "could not set the channels volume: %x\n", hr);
    return CUBEB_ERROR;
  }
...
int wasapi_init(cubeb ** context, char const * context_name)
{
  ...
  if (ctx->mmcss_module) {
    ...
    if (!(ctx->set_mm_thread_characteristics && ctx->revert_mm_thread_characteristics)) {
      LOG(nullptr, "Could not load AvSetMmThreadCharacteristics or AvRevertMmThreadCharacteristics: %x\n", GetLastError());
      FreeLibrary(ctx->mmcss_module);
    }
  } else {
    // This is not a fatal error, but we might end up glitching when
    // the system is under high load.
    LOG(nullptr, "Could not load Avrt.dll\n");
...
void stop_and_join_render_thread(cubeb_stream * stm)
{
  ...
  if (!ok) {
    LOG(stm, "Destroy SetEvent failed: %d\n", GetLastError());
  }
  ...
  if (r == WAIT_TIMEOUT) {
     ...
     * process. */
    LOG(stm, "Destroy WaitForSingleObject on thread timed out,"
        " leaking the thread: %d\n", GetLastError());
  }
  if (r == WAIT_FAILED) {
    LOG(stm, "Destroy WaitForSingleObject on thread failed: %d\n", GetLastError());
  }
...
wasapi_get_min_latency(cubeb * ctx, cubeb_stream_params params, uint32_t * latency_frames)
{
  ...
  hr = get_default_endpoint(&device, eRender);
  if (FAILED(hr)) {
    LOG(nullptr, "Could not get default endpoint: %x\n", hr);
    return CUBEB_ERROR;
  }
  ...
  SafeRelease(device);
  if (FAILED(hr)) {
    LOG(nullptr, "Could not activate device for latency: %x\n", hr);
    return CUBEB_ERROR;
  }
  ...
  if (FAILED(hr)) {
    SafeRelease(client);
    LOG(nullptr, "Could not get device period: %x\n", hr);
    return CUBEB_ERROR;
  }

  LOG(nullptr, "default device period: %lld\n", default_period);
...
handle_channel_layout(cubeb_stream * stm,  WAVEFORMATEX ** mix_format, const cubeb_stream_params * stream_params)
{
  ...
  if (hr == S_FALSE) {
    ...
    LOG(stm, "Using WASAPI suggested format: channels: %d\n", closest->nChannels);
    ...
    *reinterpret_cast<WAVEFORMATEXTENSIBLE *>(*mix_format) = hw_mix_format;
    ...
    LOG(stm, "Requested format accepted by WASAPI.\n");
  } else {
    LOG(stm, "IsFormatSupported unhandled error: %x\n", hr);
  }
}
                                 ...
                                 cubeb_stream_params * mix_params)
{
  ...
  if (devid) {
    ...
    if (FAILED(hr)) {
      LOG(stm, "Could not get %s endpoint, error: %x\n", DIRECTION_NAME, hr);
      return CUBEB_ERROR;
    }
  } else {
    hr = get_default_endpoint(&device, direction);
    if (FAILED(hr)) {
      LOG(stm, "Could not get default %s endpoint, error: %x\n", DIRECTION_NAME, hr);
      return CUBEB_ERROR;
    }
  }
  ...
  SafeRelease(device);
  if (FAILED(hr)) {
    LOG(stm, "Could not activate the device to get an audio"
  ...
  if (FAILED(hr)) {
    LOG(stm, "Could not fetch current mix format from the audio"
        " client for %s: error: %x\n", DIRECTION_NAME, hr);
    return CUBEB_ERROR;
  }
  ...
  LOG(stm, "Setup requested=[f=%d r=%u c=%u] mix=[f=%d r=%u c=%u]\n",
  ...
  hr = (*audio_client)->Initialize(AUDCLNT_SHAREMODE_SHARED,
                                   ...
                                   NULL);
  if (FAILED(hr)) {
    LOG(stm, "Unable to initialize audio client for %s: %x.\n", DIRECTION_NAME, hr);
    return CUBEB_ERROR;
  }
  ...
  if (FAILED(hr)) {
    LOG(stm, "Could not get the buffer size from the client"
        " for %s %x.\n", DIRECTION_NAME, hr);
    return CUBEB_ERROR;
  }
  ...
  if (FAILED(hr)) {
    LOG(stm, "Could set the event handle for the %s client %x.\n",
        DIRECTION_NAME, hr);
    return CUBEB_ERROR;
  }
  ...
  if (FAILED(hr)) {
    LOG(stm, "Could not get the %s client %x.\n", DIRECTION_NAME, hr);
    return CUBEB_ERROR;
  }
...
int setup_wasapi_stream(cubeb_stream * stm)
{
  ...
  if (has_input(stm)) {
    LOG(stm, "Setup capture: device=%x\n", (int)stm->input_device);
  ...
  if (has_output(stm)) {
    ...
    if (FAILED(hr)) {
      LOG(stm, "Could not get the IAudioStreamVolume: %x\n", hr);
      return CUBEB_ERROR;
    }
    ...
    if (FAILED(hr)) {
      LOG(stm, "Could not get the IAudioClock: %x\n", hr);
      return CUBEB_ERROR;
    }
  ...
  if (!stm->resampler) {
    LOG(stm, "Could not get a resampler\n");
    return CUBEB_ERROR;
  }
                   ...
                   cubeb_state_callback state_callback, void * user_ptr)
{
  ...
  if (!stm->reconfigure_event) {
    LOG(stm, "Can't create the reconfigure event, error: %x\n", GetLastError());
  ...
  if (!stm->refill_event) {
    LOG(stm, "Can't create the refill event, error: %x\n", GetLastError());
    wasapi_stream_destroy(stm);
    return CUBEB_ERROR;
  }
  ...
  if (!stm->input_available_event) {
    LOG(stm, "Can't create the input available event , error: %x\n", GetLastError());
    wasapi_stream_destroy(stm);
    return CUBEB_ERROR;
  }
  ...
  if (FAILED(hr)) {
    ...
    LOG(stm, "failed to register notification client, %x\n", hr);
  }
...
int stream_start_one_side(cubeb_stream * stm, StreamDirection dir)
{
  ...
  if (hr == AUDCLNT_E_DEVICE_INVALIDATED) {
    LOG(stm, "audioclient invalidated for %s device, reconfiguring\n",
        dir == OUTPUT ? "output" : "input");

    BOOL ok = ResetEvent(stm->reconfigure_event);
    if (!ok) {
      LOG(stm, "resetting reconfig event failed for %s stream: %x\n",
    ...
    int r = setup_wasapi_stream(stm);
    if (r != CUBEB_OK) {
      LOG(stm, "reconfigure failed\n");
      return r;
    }

    HRESULT hr2 = dir == OUTPUT ? stm->output_client->Start() : stm->input_client->Start();
    if (FAILED(hr2)) {
      LOG(stm, "could not start the %s stream after reconfig: %x\n",
          dir == OUTPUT ? "output" : "input", hr);
      return CUBEB_ERROR;
    }
  } else if (FAILED(hr)) {
    LOG(stm, "could not start the %s stream: %x.\n",
        dir == OUTPUT ? "output" : "input", hr);
    return CUBEB_ERROR;
  }
...
int wasapi_stream_stop(cubeb_stream * stm)
{
...

  {
    ...
    if (stm->output_client) {
      ...
      if (FAILED(hr)) {
        LOG(stm, "could not stop AudioClient (output)\n");
        return CUBEB_ERROR;
      }
    }
    ...
    if (stm->input_client) {
      ...
      if (FAILED(hr)) {
        LOG(stm, "could not stop AudioClient (input)\n");
        return CUBEB_ERROR;
      }
    }
                         ...
                         cubeb_device_collection ** out)
{
  ...
  *out = NULL;
  ...
  if (FAILED(hr)) {
    LOG(nullptr, "Could not enumerate audio endpoints: %x\n", hr);
    return CUBEB_ERROR;
  }

  hr = collection->GetCount(&cc);
  if (FAILED(hr)) {
    LOG(nullptr, "IMMDeviceCollection::GetCount() failed: %x\n", hr);
    return CUBEB_ERROR;
  }
  *out = (cubeb_device_collection *) malloc(sizeof(cubeb_device_collection) +
  ...
  for (i = 0; i < cc; i++) {
    ...
    if (FAILED(hr)) {
      LOG(nullptr, "IMMDeviceCollection::Item(%u) failed: %x\n", i-1, hr);
